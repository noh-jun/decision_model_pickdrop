...
public sealed partial class TabletJsonSubscriber : IDisposable
{
    ...
    private readonly IRawChunkSubscriber transport_;
    ...
    private readonly string sourceLabel_;

    // (A안) 권장 ctor: transport 주입
    public TabletJsonSubscriber(
        IRawChunkSubscriber transport,
        int byteBufferSize,
        ...
        string? sourceLabel = null)
    {
        ...
        onError_ = onError;
        sourceLabel_ = string.IsNullOrWhiteSpace(sourceLabel) ? "(unknown)" : sourceLabel;
        ...
        // transport는 raw chunk만 공급한다.
        // onError는 bool(continue)을 반환하며 transport가 이를 존중한다.
        transport_.Start(
            onChunk: OnTcpChunk,
            onError: msg => ShouldContinueOnError(msg));
    }
    ...
    private void OnTcpChunk(byte[] payloadChunk)
    {
        ...
        if (droppedBytes > 0)
        {
            ShouldContinueOnError(
                $"NOTICE: ring overflow -> oldest drop bytes={droppedBytes} (source={sourceLabel_})");
        }
    ...
    private bool ShouldContinueOnError(string errorMessage)
    {
        if (onError_ == null)
        {
            Console.Error.WriteLine(
                $"[{ClassName}][source={sourceLabel_}][pid={Environment.ProcessId}] {errorMessage}");
            return true;
        }
        ...
        {
            Console.Error.WriteLine(
                $"[{ClassName}][source={sourceLabel_}][pid={Environment.ProcessId}] on_error callback exception: {callbackException.Message}");
            return false;
        }
    }
